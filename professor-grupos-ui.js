...
#grupos table{min-width:1180px}.grp-toggle{cursor:pointer}.grp-toggle:hover{background:#f8fbfc}.grp-caret{display:inline-block;width:18px;font-weight:900;color:var(--navy);transition:transform .15s}.grp-row.open .grp-caret{transform:rotate(90deg)}.grp-summary{display:block;margin-top:4px;color:var(--muted);font-size:.67rem}.grp-stages-cell{min-width:330px}.grp-stages-wrap{display:none;gap:5px;flex-direction:column}.grp-row.open .grp-stages-wrap{display:flex}.grp-stage-line{display:flex;justify-content:space-between;align-items:center;gap:10px;padding:5px 8px;border-radius:7px;background:#f5f8f9;font-size:.68rem}.grp-stage-line b{color:#294754}.grp-stage-status{font-weight:900;white-space:nowrap}.grp-stage-status.ok{color:#2d7757}.grp-stage-status.info{color:#275f78}.grp-stage-status.danger{color:#9b2c2c}.grp-stage-status.warn{color:#956500}.grp-actions{display:flex;gap:6px;flex-wrap:wrap}.grp-delete{background:#fff!important;color:#9b2c2c!important;border:1px solid #dca7a7!important}.grp-open-btn{background:#fff!important;color:var(--navy)!important;border:1px solid var(--line)!important}
</style>`;
...
async function renderDetalhesGrupos(){
 ...
 [...body.querySelectorAll('tr')].forEach(tr=>{
   ...
   tr.classList.add('grp-row');while(tr.cells.length>5)tr.deleteCell(5);
   ...
   tr.cells[1].classList.add('grp-toggle');tr.cells[1].innerHTML=`<span class="grp-caret">▶</span><b>${escG(g.nome||'Grupo')}</b><span class="grp-summary">Clique para mostrar/recolher etapas • ${done}/${arr.length} concluídas</span>`;
   const td=tr.insertCell();td.className='grp-stages-cell grp-toggle';td.innerHTML=`<div class="grp-stages-wrap">${arr.map(x=>{const ep=epMap.get(x.etapa_id);return `<div class="grp-stage-line"><b>${escG(ep?.nome||'Etapa')}</b><span class="grp-stage-status ${sclass(x.status)}">${escG(x.status||'—')}</span></div>`}).join('')||'<span class="sub">Sem etapas vinculadas.</span>'}</div><span class="grp-summary">${arr.length} etapas — clique no grupo para expandir</span>`;
   ...
   tr.cells[1].onclick=toggle;td.onclick=toggle;act.querySelector('.grp-open-btn').onclick=toggle;act.querySelector('.grp-delete').onclick=()=>excluirGrupoRobusto(g);
 });
}
